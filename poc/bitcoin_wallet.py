...
from .rpc import BitcoinRPC, JSONRPCError, RPC_URL
...
class BitcoinWallet:
    ...
    def __init__(self, name: str, *, rpc: BitcoinRPC = None):
        self.name = name
        if rpc:
            self.rpc = rpc
        else:
            self.rpc = BitcoinRPC(f"{RPC_URL}/wallet/{name}")
        self.addresses = []
