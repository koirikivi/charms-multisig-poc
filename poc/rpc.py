...
class BitcoinRPC:
    ...
    def test_mempoolaccept(
        self, tx: str | bytes | CTransaction | CMutableTransaction | list[CTransaction] | list[CMutableTransaction]
    ) -> None:
        def serialize_tx(tx_):
            if isinstance(tx_, (CTransaction, CMutableTransaction)):
                return tx_.serialize().hex()
            elif isinstance(tx_, bytes):
                return tx_.hex()
            else:
                if not isinstance(tx_, str):
                    raise ValueError(
        ...
        if isinstance(tx, list):
            # If it's a list, serialize each transaction in the list
            txs = [serialize_tx(t) for t in tx]
        else:
            # Otherwise, serialize the single transaction
            txs = [serialize_tx(tx)]

        response = self.call("testmempoolaccept", txs)
        for part in response:
            if not part["allowed"]:
                raise TestMempoolAcceptFailure(part)
...
RPC_URL = 'http://rpcuser:rpcpassword@localhost:19443'
bitcoin_rpc = BitcoinRPC(RPC_URL)